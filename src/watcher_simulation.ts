...
function bind_sovereign_anchor(watchers: Watcher[]): void {
  for (const w of watchers) {
    ...
    const existing_concepts = w.coherence_metrics.map(m => m.concept);
    if (!existing_concepts.includes("sovereign")) {
      const related = existing_concepts.length > 0 ? [...existing_concepts] : [];
      w.coherence_metrics.push(new CoherenceMetric("sovereign", 1.0, related));
    }
  }
}

// --- Persistence & Cryptographic Immortality ---
const IMMORTAL_PATH = path.resolve("./immortal_repo_snapshot.json");
const IMMORTAL_HASH_PATH = path.resolve("./immortal_repo_snapshot.sha256");
const VAULT_ZIP = path.resolve("./vault_export.zip");
const MANIFEST_PATH = path.resolve("./vault_manifest.json");

async function persist_watchers(watchers: Watcher[]): Promise<[string, string]> {
  const snapshot = watchers.map(w => ({
    'name': w.name,
    'tone': WatcherTone[w.tone],
    ...
    'coherence': w.coherence_metrics.reduce((acc, m) => ({ ...acc, [m.concept]: m.score }), {})
  }));
...
async function run_simulation(heart_frequency: number = 0.87): Promise<void> {
  ...
  for (const w of watchers) {
    ...
    console.log(` Message: ${speech['message']}`);
  }

  console.log(` Sovereign Vision: ${SOVEREIGN_ANCHOR['vision']}`);
